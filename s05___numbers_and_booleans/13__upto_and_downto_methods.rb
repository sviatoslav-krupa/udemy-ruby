...
# Returns first value
5.downto(1) { |count| puts count } # => 5 (prints 5, 4, 3, 2, 1)

# Iterates from 1 to 5 with step 1 (including 1 and 5)
# Returns first value
1.upto(5) { |count| puts count } # => 1 (prints 1, 2, 3, 4, 5)

# Edge cases:
1.downto(5) { |count| puts count } # => 1 (no iterations here)
5.upto(1) { |count| puts count } # => 5 (no iterations here)

# Iterate to the closest Integer
# Returns first value
10.downto(5.2) { |count| puts count } # => 10 (prints 10, 9, 8, 7, 6)
5.upto(10.3) { |count| puts count } # => 5 (prints 5, 6, 7, 8, 9, 10)
